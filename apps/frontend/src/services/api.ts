...
export const fixturesApi = {
  ...
  getByDateRange: (startDate: string, endDate: string) =>
    api.get(/fixtures, { params: { startDate, endDate } }).then(r => r.data),
  ...
  getComplete: (fixtureId: number) =>
    api.get(/fixtures//complete).then(r => r.data),

  getLive: () =>
    api.get(/live-fixtures).then(r => r.data),

  refreshScores: (date: string) =>
    api.post(/fixtures/refresh, { date }).then(r => r.data),

  getH2H: (homeTeamId: number, awayTeamId: number) =>
    api.get(/fixtures/h2h, { params: { homeTeamId, awayTeamId } }).then(r => r.data),

  getStandings: (leagueId: number, season: number) =>
    api.get(/leagues//standings, { params: { season } }).then(r => r.data),
};
...
export const betBuilderApi = {
  getDaily: () =>
    api.get(/bet-builder).then(r => r.data),

  getHistory: (params?: any) =>
    api.get(/bet-builder/history, { params }).then(r => r.data),
};

// Golden Bets API
export const goldenBetApi = {
  getAll: (params?: any) =>
    api.get(/golden-bets, { params }).then(r => r.data),
};
...
export const bettingInsightsApi = {
  getAll: (params?: any) =>
    api.get(/betting-insights, { params }).then(r => r.data),
};
