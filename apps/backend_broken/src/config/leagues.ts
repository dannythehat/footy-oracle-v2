...
export const LEAGUE_CONFIGS: Record<number, LeagueConfig> = {
  // England
  39: {
    id: 39,
    name: 'Premier League',
 ...
 */
export function getLeagueLogo(leagueId: number): string | null {
 ...
 */
export function getSupportedLeagueIds(): number[] {
  return Object.keys(LEAGUE_CONFIGS).map(Number);
}

/**
 * Check if a league is supported
 */
export function isLeagueSupported(leagueId: number): boolean {
  return leagueId in LEAGUE_CONFIGS;
}
